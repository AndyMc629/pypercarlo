...
# -*- coding: utf-8 -*-
"""
Created on Sun May 14 10:21:52 2017
...
class Simulation:
    ...
    def step(self):
        for s in range(self.L*self.L):
            ...
            randSite = [self.randint(self.L), self.randint(self.L)]
            
            # Measure local energy e = -s_k * sum_{l nn k} s_l
            localE = -self.spins[randSite]*(self.spins[randSite[0]+1, randSite[1]],
                                self.spins[randSite[0]-1,randSite[1]],
                                self.spins[randSite[0], randSite[1]+1],
                                self.spins[randSite[0], randSite[1]-1])
            
            # Flip s_k with probability exp(2 beta e)
            prob = self.exp_table[localE]
            if(prob<0):
                self.spins[randSite] = -self.spins[randSite] 
            else:
                if(prob>self.randint(1)):
                    self.spins[randSite] = -self.spins[randSite]
